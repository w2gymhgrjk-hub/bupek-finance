...
export async function sendSms(phone: string, message: string): Promise<SmsResult> {
  // Use mock in development ONLY if no real API key is configured
  const hasRealKey = env.AT_API_KEY && env.AT_API_KEY !== 'none' && env.AT_API_KEY.trim() !== '';
  if (env.NODE_ENV !== 'production' && !hasRealKey) {
    logger.info(`[SMS MOCK] To: ${phone} | Message: ${message}`);
    return { success: true, messageId: `mock-${Date.now()}` };
  }
  if (!hasRealKey) {
    logger.warn('SMS: AT_API_KEY not configured — set AT_API_KEY in backend/.env');
