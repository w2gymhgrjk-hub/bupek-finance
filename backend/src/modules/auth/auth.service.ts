...
export class AuthService {
  ...
  async forgotPassword(email: string) {
    ...
    try {
      await sendPasswordResetEmail(user.email, `${user.firstName} ${user.lastName}`, rawToken);
    } catch (emailErr) {
      // If email fails, still return success to avoid breaking the flow in dev
      logger.warn(`Password reset email failed for ${email}. Token: ${rawToken}`);
    }
    return rawToken;
  }
  ...
  async resetPassword(token: string, newPassword: string) {
    ...
    const user = await prisma.user.findUnique({ where: { id: resetToken.userId }, select: { email: true, firstName: true, lastName: true } });
    ...
    if (user) sendPasswordChangedEmail(user.email, `${user.firstName} ${user.lastName}`).catch(() => {});
  }
